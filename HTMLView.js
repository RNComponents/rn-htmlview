...
function htmlToElement(rawHtml, opts, done) {
  function domToElement(dom, parent) {
    ...
    return dom.map((node, index, list) => {
      ...
      if (node.type == 'text') {
        return (
          <Text key={index} style={[parent ? opts.styles[parent.name] : null, {flex:1,flexWrap:'wrap'}]}>
...
var HTMLView = React.createClass({
  ...
  render() {
    if (this.state.element) {
      return <View style={{flexDirection:this.props.direction || 'column'}} children={this.state.element} />
    }
    return <Text />
  }
})
